...
namespace DataTransferObjects.DataTransferObjects.HrDTOs
{
    ...
    public class PersonCuDto : BaseDto<PersonCuDto, CrePerson, long>, IValidatableObject
    {
        ...
        {
            mappingExpression.ForMember(dest => dest.FPeopleTypesId, conf => conf.MapFrom(src => src.TypeId));
        }

        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (TypeId == (int)GlobalEnums.PeopleType.Individual && IndividualPerson is null)
                yield return new ValidationResult("اطلاعات شخص حقیقی ناقص است", new[] { nameof(IndividualPerson) });

            if (TypeId == (int)GlobalEnums.PeopleType.Company && Company is null)
    ...
    public class PersonListDto : BaseListDto<PersonListDto, CrePerson, long>
    {
        ...
        public override void CustomMappings(IMappingExpression<CrePerson, PersonListDto> mappingExpression)
        {
            ...
            mappingExpression
                .ForMember(dest => dest.FullName, opt => opt.MapFrom(src => src.IndividualPerson == null ? src.Company.Title : string.Concat((string?)src.IndividualPerson.FirstName, (string?)" ", (string?)src.IndividualPerson.LastName)));
    ...
    public class PersonBriefListDto : BaseDto<PersonBriefListDto, CrePerson, long>
    {
        ...
        {
            mappingExpression.ForMember(dest => dest.FullName, opt => opt.MapFrom(src => src.IndividualPerson == null ? src.Company.Title : string.Concat((string?)src.IndividualPerson.FirstName, (string?)" ", (string?)src.IndividualPerson.LastName)));
    ...
    public class PersonSearchDto : BaseSearchDto, IHaveCustomExpression<CrePerson, PersonSearchDto, long>
    {
        ...
        public Expression<Func<CrePerson, bool>> GenerateExpression(PersonSearchDto dto)
        {
            ...
            {
                expressions.Add(src => src.FPeopleTypesId == (int)GlobalEnums.PeopleType.Individual ?
            ...
            {
                expressions.Add(src => src.FPeopleTypesId == int.Parse(PersonTypeId));
            }
    ...
    //    public IndividualCuDto IndividualPerson { get; set; }
    //    public CompanyCuDto Company { get; set; }
    //    public List<PeopleAddressCuDto> PeopleAddresses { get; set; }

    //    public override void CustomMappings(IMappingExpression<CrePerson, PersonFullDto> mappingExpression)
    //    {
    //        mappingExpression
    //            .ForMember(dest => dest.TypeId, conf => conf.MapFrom(src => src.FPeopleTypesId));
    //    }
