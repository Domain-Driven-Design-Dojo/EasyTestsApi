...
namespace Entities.PersonModels
{
    ...
    public class CreCompanyConfiguration : IEntityTypeConfiguration<CreCompany>
    {
        public void Configure(EntityTypeBuilder<CreCompany> builder)
        {
            ...
            //builder.HasOne(p => p.Person).WithMany(c => c.Companies).HasForeignKey(c => c.FPersonsId);
            builder.HasOne(p => p.Person).WithOne(c => c.Company).HasForeignKey<CreCompany>(c => c.FPersonsId);
        }
    }
}
